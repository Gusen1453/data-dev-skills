...
| **data-grill** | 数据开发设计先行：建表/管道/接口/质检动手前一次一问敲定需求与关键决策，产出轻量决策记录作为执行器输入前提 |
| **data-solid** | 数仓结构审查：分层、粒度主键、时态/PIT、契约一致性、复用五面，产出结构审查点与改进建议 |

不确定使用哪个 skill 时，从 **using-data-dev** 开始；如需设计方法先行，可用 **/data-grill** 敲定需求、**/data-solid** 审查数仓结构。
